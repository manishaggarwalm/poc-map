...
const Header = (props) => {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <button className={`navbar-toggler ${!isOpen ? 'collapsed' : ''}`} type="button" onClick={() => handleMenu(!isOpen)}>
      ...
      <div className={`collapse navbar-collapse ${isOpen ? 'show' : ''}`} id="navbarNav">
        <ul className="navbar-nav">
          ...
          <NavItem currentPath={pathname} title="Administration" to="/administration" />
        </ul>
      </div>
    </nav>
  );
